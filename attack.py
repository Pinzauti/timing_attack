...
def guess_letter(length, result):
    ...
    password c. :param length: It is the number of characters which we still don't know and we
    are going to guess in this and other iterations. :param result: It is a string containing the
    previous guessed characters by this same function. :return: It returns a single character
    which we think is equal to the one in the same position in c (c is the password to guess).
    """
    attack_time = {}
...
def attack(length):
    ...
    to the param length) with the characters guessed by the guess_letter function. :param length:
    It is the guessed length of the password, generated by the function guess_length. :return: It
    returns a tuple, the first value is the guess of the password, the second value is the time
    that was necessary to crack it.
    """
